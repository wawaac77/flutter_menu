...
//  ClubSIM
//
//  Created by Alice Jin on 5/7/2018.
...
@interface LSEmojiFlyView ()
@property(nonatomic,strong) UIColor *strokeColor;
@property(nonatomic,strong) UIColor *fillColor;
@end

@implementation LSEmojiFlyView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        //self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"emoji_hug.png"]];
...
- (void)animateInView:(UIView *)view withImageName:(NSString *)imageName {
...
//    else {
//        [[UIImage imageNamed:@"emoji_like.png"] drawInRect:self.bounds];
//    }
    ...
    NSTimeInterval bloomDuration = 0.5;
 
    
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation)) {
        totalAnimationDuration = 1.5;
    }
    ...
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
    {
        ...
        [UIView animateWithDuration:bloomDuration delay:0.0 usingSpringWithDamping:0.6 initialSpringVelocity:0.8 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.transform = CGAffineTransformIdentity;
            self.alpha = 0.9;
        } completion:NULL];
    }
...
//    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
//    {
//        [heartTravelPath moveToPoint:CGPointMake(self.center.x, self.center.y - 20)];
//    }
//    else {
//
//    }
    ...
    //random end point
    CGPoint endPoint = CGPointMake(heartCenterX + (rotationDirection) * arc4random_uniform(2*heartSize), 40 + arc4random_uniform(viewHeight/6.0));
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
    {
        endPoint = CGPointMake(heartCenterX + (rotationDirection) * arc4random_uniform(2*heartSize), 40 + arc4random_uniform(viewHeight/1000.0));
    }
...
////        UIImage *heartImage = [UIImage imageNamed:@"emoji_like"];
